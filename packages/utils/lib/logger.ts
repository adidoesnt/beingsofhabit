...
export class Logger {
    ...
    private getTimestamp() {
        const date = new Date();
        const dateString = NODE_ENV === "DEV" ? date.toLocaleString() : date.toISOString();
        return `[${dateString}]`;
    }
    ...
    info(message: string) {
        const prefix = colourMap.get(LogLevel.INFO)!(`${this.getTimestamp()} ${this.name}`);
    ...
    debug(message: string, ...params: any[]) {
        const prefix = colourMap.get(LogLevel.DEBUG)!(`${this.getTimestamp()} ${this.name}`);
        const formattedMessage = `${prefix} ${message}`;
    ...
    warn(message: string) {
        const prefix = colourMap.get(LogLevel.WARN)!(`${this.getTimestamp()} ${this.name}`);
        const formattedMessage = `${prefix} ${message}`;
    ...
    error(message: string, error: Error) {
        const prefix = colourMap.get(LogLevel.ERROR)!(`${this.getTimestamp()} ${this.name}`);
        const formattedMessage = `${prefix} ${message}`;
