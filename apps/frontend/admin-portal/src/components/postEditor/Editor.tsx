...
export const Editor = ({ post }: { post: Post }) => {
  ...
  const getPresignedUrl = useCallback(
    async (file: File) => {
      ...
      const response = await apiClient.get(`/bucket/presigned-url?${query}`);
  ...
  const uploadImage = useCallback(async (file: File, presignedUrl: string) => {
  ...
  const savePost = useCallback(
    async (formData: z.infer<typeof formSchema>) => {
      setIsSaving(true);
      try {
        await handleImageUpload(formData.headerImage[0]);
        const { headerImage: _headerImage, ...formDataWithoutHeaderImage } =
          formData;
