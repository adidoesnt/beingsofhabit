...
export const SessionExpiryAlert = () => {
    const { showSessionExpiryPopup, setSessionExpiryWithMaxAge, setUser, setShowSessionExpiryPopup } = useAuth();
    const [refreshFailed, setRefreshFailed] = useState(false);
    const [title, setTitle] = useState("Session Expired");
    const [description, setDescription] = useState("Your session has expired. Please refresh to continue.");
    ...
    const buttons = useMemo(() => {
        return (
            <>
                {!refreshFailed && <Button className="w-fit" onClick={refreshSession}>Refresh</Button>}
                <Button className="w-fit bg-red-600 text-white" onClick={logout}>Logout</Button>
            </>
        );
